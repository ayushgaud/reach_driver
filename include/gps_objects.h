...
struct GPS_State
{
  ...
  uint8_t fix_count;                  // 8 bit count of fix messages processed, used for
                                      // periodic processing
  ...
  bool have_vertical_velocity : 1;    ///< does GPS give vertical velocity? Set to
                                      ///< true only once available.
  bool have_speed_accuracy : 1;       ///< does GPS give speed accuracy? Set to true
                                      ///< only once available.
  bool have_horizontal_accuracy : 1;  ///< does GPS give horizontal position
                                      ///< accuracy? Set to true only once
                                      ///< available.
  bool have_vertical_accuracy : 1;    ///< does GPS give vertical position
                                      ///< accuracy? Set to true only once
                                      ///< available.
  ...
  uint32_t rtk_age_ms;               ///< GPS age of last baseline correction in milliseconds
                                     ///< (0 when no corrections, 0xFFFFFFFF indicates
                                     ///< overflow)
  uint8_t rtk_num_sats;              ///< Current number of satellites used for RTK calculation
  uint8_t rtk_baseline_coords_type;  ///< Coordinate system of baseline. 0 ==
                                     ///< ECEF, 1 == NED
  int32_t rtk_baseline_x_mm;         ///< Current baseline in ECEF x or NED north
                                     ///< component in mm
  int32_t rtk_baseline_y_mm;         ///< Current baseline in ECEF y or NED east
                                     ///< component in mm
  int32_t rtk_baseline_z_mm;         ///< Current baseline in ECEF z or NED down
                                     ///< component in mm
  uint32_t rtk_accuracy;             ///< Current estimate of 3D baseline accuracy
                                     ///< (receiver dependent, typical 0 to 9999)
  int32_t rtk_iar_num_hypotheses;    ///< Current number of integer ambiguity
                                     ///< hypotheses
};
