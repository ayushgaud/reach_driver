...
{	
	...
	ros::Publisher gps_vel_pub = n.advertise<geometry_msgs::TwistWithCovarianceStamped>("fix_velocity", 100);
	...
	while(ros::ok())
	{
		...
		if(states.fix_count > last_fix)
		{
			...
			velocity_msg.twist.twist.angular.z = states.ground_course; // Heading in degrees

			velocity_msg.twist.covariance = { states.horizontal_accuracy, 0,   0,  0,  0,  0,
						                      0,   states.horizontal_accuracy, 0,  0,  0,  0,
						                      0,   0,   states.vertical_accuracy, 0, 0,  0,
						                      0,   0,   0,  1e6, 0, 0,
						                      0,   0,   0,  0,  1e6, 0,
						                      0,   0,   0,  0,  0,  1e6};
			gps_pub.publish(msg);
